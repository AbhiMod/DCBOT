...
def welcomepic(pic, user, chat, id, uname):
    background = Image.open("DCBOT/assets/bg2.jpg")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((450, 450))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('DCBOT/assets/font.ttf', size=40)
    welcome_font = ImageFont.truetype('DCBOT/assets/font.ttf', size=60)
...
➖➖➖➖➖➖➖➖➖➖➖➖**
""",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"⦿ ᴀᴅᴅ ᴍᴇ ⦿", url=f"https://t.me/DCBOTBot?startgroup=true")]])
        )
    except Exception as e:
        LOGGER.error(e)
    try:
