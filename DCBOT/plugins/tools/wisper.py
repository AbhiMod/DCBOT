    ...
    if len(data.split()) < 2:
        mm = [
            InlineQueryResultArticle(
                ...
                thumb_url="https://telegra.ph/file/ded183ca55ebb424a6e3f.jpg",
...
async def whispes_cb(_, query):
    ...
    if user_id not in [from_user, to_user, 6204761408]:
        try:
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💒 Whisper",
            description=f"@DCBOTBot [USERNAME | ID] [TEXT]",
            input_message_content=InputTextMessageContent(f"**📍Usage:**\n\n@DCBOTBot (Target Username or ID) (Your Message).\n\n**Example:**\n@DCBOTBot @username hii baby"),
            thumb_url="https://telegra.ph/file/ded183ca55ebb424a6e3f.jpg",
            reply_markup=switch_btn
        )
    ]
